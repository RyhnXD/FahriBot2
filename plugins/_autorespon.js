    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? `${namabot} tidak aktif` : banned ? 'kamu dibanned' : `ngapain tag"🗿?`,
                footer,
...
╠═〘 ${namabot} 〙 ═`.trim(), footer, 'Dana', '#viadana', 'GoPay', '#viagopay', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`ѕнιησмιуα вσтz || Run time ${uptime} ||  Mode: ${global.opts['self'] ? 'Ƥяιѵαтɛ' : setting.groupOnly ? 'Hanya Grup' : 'Ƥʋвℓικ'}`).catch(_ => _)
